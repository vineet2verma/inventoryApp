...
export default function PaymentMasterPage() {
  ...
  useEffect(() => {
    setrightread(user.user?.ppaymenttype.includes("read"));
    setrightcreate(user.user?.ppaymenttype.includes("create"));
    setrightedit(user.user?.ppaymenttype.includes("update"));
    setrightdelete(user.user?.ppaymenttype.includes("delete"));
  }, [user]);
