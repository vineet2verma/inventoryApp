...
import BreakageMast from "@/app/api/models/breakageMast";

// GET all inventory records
export async function GET() {
  try {
    await connectToDatabase();
    const records = await BreakageMast.find().sort({ createdAt: -1 }); // Newest first
...
export async function POST(req) {
  ...
  try {
    ...
    const newRecord = new BreakageMast(data);
    await newRecord.save();

    return NextResponse.json({ message: "Client record created successfully" }, { status: 200 });
  } catch (err) {
    console.error(err);
...
export async function PUT(req) {
  try {
    ...
    const updated = await BreakageMast.findByIdAndUpdate(id, updatedData, { new: true });
...
export async function DELETE(req) {
  try {
    ...
    const deleted = await BreakageMast.findByIdAndDelete(id);
