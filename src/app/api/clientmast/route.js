...
import ClientMaster from "@/app/api/models/clientDB"; // Adjust the import path as necessary

// GET all inventory records
export async function GET() {
  try {
    await connectToDatabase();
    const records = await ClientMaster.find().sort({ createdAt: -1 }); // Newest first
...
export async function POST(req) {
  ...
  try {
    ...
    const newRecord = new ClientMaster(data);
...
export async function PUT(req) {
  try {
    ...
    const updated = await ClientMaster.findByIdAndUpdate(id, updatedData, { new: true });
