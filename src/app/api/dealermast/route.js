...
import dealerMast from "@/app/api/models/dealerMast";
...
export async function POST(req) {
  ...
  try {
    ...
    const newRecord = new dealerMast(data);
...
export async function PUT(req) {
  try {
    ...
    const updated = await dealerMast.findByIdAndUpdate(id, updatedData, { new: true });
...
export async function DELETE(req) {
  try {
    ...
    const deleted = await dealerMast.findByIdAndDelete(id);
