...
import { Pencil, Trash2, House, PackagePlus } from "lucide-react";
...
export default function DealerMastPage() {
  ...
  useEffect(() => {
    setrightread(user.user?.pcustomer.includes("read"));
    setrightcreate(user.user?.pcustomer.includes("create"));
    setrightedit(user.user?.pcustomer.includes("update"));
    setrightdelete(user.user?.pcustomer.includes("delete"));
  }, [user]);
