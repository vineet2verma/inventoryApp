...
export default function LocationMasterPage() {
    ...
    const [form, setForm] = useState({ location: '', createdBy: '', status: 'Active' });
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen">
            ...
            {statusMsg && (
                <div className={`mb-4 p-3 rounded-xl flex items-center space-x-2 ${statusMsg.success ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {statusMsg.success ? <CheckCircle className="w-5 h-5" /> : <XCircle className="w-5 h-5" />}
            ...
            {modalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-2xl p-6 shadow-xl w-full max-w-md">
                        <h3 className="text-xl font-semibold mb-4">{form.id ? 'Edit Location' : 'Add Location'}</h3>
